...
wsj0=/home/nazgul/Unistuff/voice/lab2/WSJ0_2015
...
if [ $stage -le 0 ]; then
  ...
  for x in test dev train; do
    steps/make_mfcc.sh --nj 8 data/$x || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  if $train; then
    steps/train_mono.sh --boost-silence 1.25 --nj 2 --cmd "$train_cmd" \
      data/train data/lang_nosp exp/mono0a || exit 1;
  fi
  ...
  if $decode; then
    utils/mkgraph.sh data/lang_nosp_test_tgpr exp/mono0a exp/mono0a/graph_nosp_tgpr && \
      steps/decode.sh --nj 2 --cmd "$decode_cmd" exp/mono0a/graph_nosp_tgpr \
        data/dev exp/mono0a/decode_nosp_tgpr_dev && \
      steps/decode.sh --nj 2 --cmd "$decode_cmd" exp/mono0a/graph_nosp_tgpr \
        data/test exp/mono0a/decode_nosp_tgpr_test
  fi
fi
...
if [ $stage -le 2 ]; then
  ...
  if $train; then
    ...
    steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" 2000 10000 \
      data/train data/lang_nosp exp/mono0a_ali exp/tri1 || exit 1;
  fi
  ...
  if $decode; then
    ...
    for data in dev test; do
      ...
      steps/decode.sh --nj $nspk --cmd "$decode_cmd" exp/tri1/graph_nosp_tgpr \
        data/${data} exp/tri1/decode_nosp_tgpr_${data} || exit 1;
      ...
      for mode in 1 2 3 4 5; do
        steps/lmrescore.sh --mode $mode --cmd "$decode_cmd" \
          data/lang_nosp_test_{tgpr,tg} data/${data} \
...
if [ $stage -le 3 ]; then
  ...
  if $train; then
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" \
      ...
      data/train data/lang_nosp exp/tri1_ali exp/tri2b || exit 1;
  fi
  ...
  if $decode; then
    ...
    for data in dev test; do
      ...
      steps/decode.sh --nj ${nspk} --cmd "$decode_cmd" exp/tri2b/graph_nosp_tgpr \
        data/${data} exp/tri2b/decode_nosp_tgpr_${data} || exit 1;
      ...
      steps/decode_biglm.sh --nj ${nspk} --cmd "$decode_cmd" \
        ...
        data/${data} exp/tri2b/decode_nosp_tgpr_${data}_tg_biglm

       # baseline via LM rescoring of lattices.
      steps/lmrescore.sh --cmd "$decode_cmd" \
        data/lang_nosp_test_tgpr/ data/lang_nosp_test_tg/ \
        data/${data} exp/tri2b/decode_nosp_tgpr_${data} \
      ...
      local/score_mbr.sh --cmd "$decode_cmd"  \
         data/${data}/ data/lang_nosp_test_tgpr/ \
         exp/tri2b/decode_nosp_tgpr_${data}_tg_mbr
    done
  fi
...
if [ $stage -le 4 ]; then
  ...
  if $train; then
    steps/align_si.sh  --nj 1 \
      data/train data/lang_nosp exp/tri2b exp/tri2b_ali  || exit 1;

    steps/train_sat.sh --cmd "$train_cmd" 4200 40000 \
      data/train data/lang_nosp exp/tri2b_ali exp/tri3b || exit 1;
  fi
  ...
  if $decode; then
    ...
    for data in dev test; do
      ...
      steps/decode_fmllr.sh --nj ${nspk} --cmd "$decode_cmd" \
        exp/tri3b/graph_nosp_tgpr data/${data} \
        exp/tri3b/decode_nosp_tgpr_${data} || exit 1;
      steps/lmrescore.sh --cmd "$decode_cmd" \
        data/lang_nosp_test_tgpr data/lang_nosp_test_tg \
        data/${data} exp/tri3b/decode_nosp_{tgpr,tg}_${data} || exit 1

      # # decode with big dictionary.
      # steps/decode_fmllr.sh --cmd "$decode_cmd" --nj 8 \
      #   exp/tri3b/graph_nosp_bd_tgpr data/${data} \
      ...
      #   data/${data} exp/tri3b/decode_nosp_bd_tgpr_${data}{,_fg} || exit 1;
    done
  fi
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/get_prons.sh --cmd "$train_cmd" \
    data/train data/lang_nosp exp/tri3b || exit 1;
  ...
  for lm_suffix in bg tg tgpr; do
...
if [ $stage -le 6 ]; then
  ...
  if $train; then
    steps/train_sat.sh  --cmd "$train_cmd" 4200 40000 \
      data/train data/lang exp/tri3b exp/tri4b || exit 1;
  fi
  ...
  if $decode; then
    ...
    for data in dev test; do
      nspk=$(wc -l <data/${data}/spk2utt)
      steps/decode_fmllr.sh --nj ${nspk} --cmd "$decode_cmd" \
        exp/tri4b/graph_tgpr data/${data} \
        exp/tri4b/decode_tgpr_${data} || exit 1;
      steps/lmrescore.sh --cmd "$decode_cmd" \
        data/lang_test_tgpr data/lang_test_tg \
        data/${data} exp/tri4b/decode_{tgpr,tg}_${data} || exit 1

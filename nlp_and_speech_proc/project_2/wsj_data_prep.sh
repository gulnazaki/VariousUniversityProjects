...
if [ ! -d links/11-13.1 -o ! -d links/11-15.1 -o ! -d links/11-4.1 ]; then
...
cat links/11-13.1/wsj0/doc/indices/train/tr_l_wv1.ndx | \
 $local/ndx2flist.pl $* | sort | \
 grep -i 11-4.1/wsj0/ > train.flist

nl=`cat train.flist | wc -l`
[ "$nl" -eq 1806 ] || echo "Warning: expected 1806 lines in train.flist, got $nl"
...
# have to add .wv1
cat links/11-13.1/wsj0/doc/indices/test/nvp/ld_et_05.ndx | \
  $local/ndx2flist.pl $* |  awk '{printf("%s.wv1\n", $1)}' | \
  sort > test.flist

# Nov'92 (330 utts, 5k vocab)
cat links/11-13.1/wsj0/doc/indices/test/vp/ld_et_05.ndx | \
  $local/ndx2flist.pl $* |  awk '{printf("%s.wv1\n", $1)}' | \
  sort > dev.flist
...
cp links/11-13.1/wsj0/doc/lng_modl/vocab/wfl_64.lst $lmdir
...
cp links/11-13.1/wsj0/doc/lng_modl/base_lm/bcb20onp.z $lmdir/lm_bg.arpa.gz || exit 1;
...
cp links/11-13.1/wsj0/doc/lng_modl/base_lm/tcb05cnp.z $lmdir/lm_tg.arpa.gz || exit 1;
chmod u+w $lmdir/lm_tg.arpa.gz
gunzip $lmdir/lm_tg.arpa.gz
tail -n 4328839 $lmdir/lm_tg.arpa | gzip -c -f > $lmdir/lm_tg.arpa.gz
rm $lmdir/lm_tg.arpa

...
ax1.set_ylim(min(theor_Axis) - 0.2 * min(theor_Axis), max(theor_Axis) + 0.08 * max(theor_Axis))
...
ax2.set_ylim(min(pract_Axis) - 0.2 * min(pract_Axis), max(pract_Axis) + 0.08 * max(pract_Axis))
...
plt.savefig("./nodes/utilisation.png",bbox_inches="tight")

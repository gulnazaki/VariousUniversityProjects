...
with open("./data/test_greng.txt","r") as greng, open("./data/test_gr.txt","r") as gr, open("./ofinaltranslation.txt","w") as tran, open("./ofinalaccuracy.txt","w") as stats:
	...
	for line in range(len(greng)):
		for word in range(len(greng[line])):
			...
			system('fstcompose W.fst oFinal.fst oFinal.fst')
			system('fstshortestpath oFinal.fst oFinal.fst')
			system('fstprint --isymbols=syms.txt --osymbols=syms.txt oFinal.fst > temp.txt')
